...
- The conformance lane runs against `ucp-sdk` 0.5.0 — the UCP `2026-08-25` model set — and is **blocking for seven modules**. `scripts/run-conformance.sh` hard-resets the checkout before patching (`git checkout <sha>` keeps working-tree modifications, so hand-applied fixes silently survived and a "pinned" result was nothing of the kind), applies `docs/upstream/*.patch` itself, and provisions a signing key before booting the merchant. A clean clone now reports **53 passed / 11 failed** twice in a row with identical failure sets; every remaining failure is outside this repository ([#157](https://github.com/agentic-commerce-alliance/ucp-php-sdk/pull/157))
...
- The merchant example resolves every environment variable through one reader that also consults `getenv()`. `$_ENV` is populated only when `variables_order` says so, and under `php -S` it commonly is not — so the console and the HTTP server resolved **different state directories from the same configuration**, and a signing key generated on the command line landed in a database the server never opened. The server then declined to sign anything while reporting no configuration problem. `bin/console` is new: the SDK ships signing-key rotation, storage cleanup and nonce purging as console commands and this example had no entry point for any of them ([#157](https://github.com/agentic-commerce-alliance/ucp-php-sdk/pull/157))
