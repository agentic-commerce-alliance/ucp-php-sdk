...
- The conformance lane is **blocking** for seven modules — `checkout_lifecycle`, `discount`, `protocol`, `simulation_url_security`, `totals`, `validation` and `webhook` — listed in `tests/conformance/enforced-modules.txt`. The baseline moved from 5 passed / 59 failed to **53 passed / 11 failed**, and every remaining failure is outside this repository: three upstream suite defects, one suite model-version blocker, one operation the upstream OpenAPI does not define, and one item belonging to the signature lane. None of those are enforced, because blocking on something unfixable here teaches people to ignore a red check ([#157](https://github.com/agentic-commerce-alliance/ucp-php-sdk/pull/157))
