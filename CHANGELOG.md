...
- A `keyid` may be an RFC 7638 JWK thumbprint as well as an operator's label. A `kid` is a name someone chose — two businesses both calling their key `"default"` is not a collision anyone detects, and renaming a key invalidates every signature it ever sent — while a thumbprint is derived from the key material, so it identifies the same key everywhere. Web-bot-auth names keys this way. The label is still tried first, since that is what existing peers send, and a key whose material cannot be thumbprinted simply does not match one rather than failing the whole key set ([#162](https://github.com/agentic-commerce-alliance/ucp-php-sdk/pull/162))
- `Signature-Agent` resolution and `tag="web-bot-auth"`. When a signature carries that tag, the keys published at the agent's own directory URL are fetched and added to the candidate set, so a bot whose keys this business has never seen can still be verified — combined with RFC 7638 key ids, which is how web-bot-auth names them. The URL arrives in a request header, so it goes through the same allow-list, address pinning, redirect refusal and streamed size cap as a platform profile, and the fetch happens **only** when the tag asks for it: a `Signature-Agent` header alone would otherwise make every inbound request an outbound one. UCP's default shape carries `tag="sig1"`, which is accepted alongside it -- an early version of this listed only `web-bot-auth` and so refused the baseline every business must accept, trading one interop break for another. A genuinely unrecognised tag is still refused rather than ignored, because a tag names requirements and verifying under one this implementation does not apply reports a check as passed that was never performed. Directories are cached in `ucp_agent_key_directory_cache` — its own table, since a profile eviction should not silently drop an agent's keys ([#165](https://github.com/agentic-commerce-alliance/ucp-php-sdk/pull/165))
...
- `PublicSigningKey` accepts OKP keys rather than only tolerating them. `kty` was checked against the single value `EC`, which was correct while EC was the only thing this SDK could verify with and became a rejection of the key type it now signs with itself. Key type and curve are validated as a pair — an `EC` key on `Ed25519` describes a point on a curve that type does not use — and an Edwards key's PEM is built from `x` alone, since RFC 8037 gives it no `y` ([#165](https://github.com/agentic-commerce-alliance/ucp-php-sdk/pull/165))
